...
export default function AudioBayanaat() {
  ...
  // ⚠️ Ensure all 'url' fields use paths relative to the /public directory
  const bayanat = [
  ...
  const handlePlay = (bayan) => {
    if (currentAudio?.id === bayan.id && isPlaying) {
      ...
      // Removed the setTimeout; playing should be instant when the source is set.
      // The useEffect hook handles loading and playing when currentAudio changes.
    }
  };
    ...
    if (currentAudio) {
      if (audioRef.current) {
        ...
        audioRef.current.play().then(() => {
          ...
          // Handle potential errors like blocked auto-play
          setIsPlaying(false);
        });
      }
    }
  }, [currentAudio]); // Triggered when a new audio is selected
  ...
  useEffect(() => {
    ...
    // Add event listeners for play/pause events to keep state sync'd
  ...
  }, [currentAudio]); // Re-run effect when audio source element is set/changed
    ...
    <div className="bg-white min-h-screen py-16 px-4 sm:px-6 lg:px-12">
      ...
      <div className="max-w-6xl mx-auto text-center mb-10">
        <h3 className="text-3xl md:text-4xl font-extrabold text-emerald-800 relative inline-block">
          آڈیو بیانات
          {/* ❌ Left-to-Right layout flip: left-1/2 changed to start-1/2 */}
          <span className="absolute -bottom-2 start-1/2 -translate-x-1/2 w-24 h-1 bg-emerald-400 rounded-full"></span>
        </h3>
      </div>
      ...
      <div className="hidden md:block overflow-x-auto bg-white shadow-lg rounded-xl">
        <table className="w-full text-right border-collapse rounded-xl overflow-hidden">
          <thead className="bg-emerald-700 text-white text-sm md:text-base">
            <tr>
              {/* ❌ text-right is the default for RTL, but using logical padding p-3 for consistency */}
          ...
          <tbody>
            {filtered.map((b, idx) => (
              ...
              >
                ...
                <td className="p-3 flex gap-2 justify-end"> {/* ❌ Added justify-end for RTL button group alignment */}
                  <button 
                    className="bg-emerald-600 text-white p-2 rounded-md hover:bg-emerald-700 transition shadow"
                  >
                    <Save size={16} />
                  </button>
                  <button 
                    className="bg-gray-600 text-white p-2 rounded-md hover:bg-gray-700 transition shadow"
                  >
                    <Download size={16} />
                  </button>
                </td>
      ...
      <AnimatePresence>
        {currentAudio && (
          ...
          >
            ...
            {/* Audio element will handle source update via currentAudio */}
            <audio
              ref={audioRef}
